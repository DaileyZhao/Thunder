...
public abstract class BaseDB {
	...
	public int getCount(Pair<String, ArrayList<String>> where) {
		...
		synchronized (getDBLock()) {
			Cursor cr = null;
			try {
				cr = mSqLiteDatabase.query(
						table,
						new String[] { "count(1)" },
						where != null ? where.first : null,
						where != null && where.second != null ? where.second
	...
	public int delete(Pair<String, ArrayList<String>> where) {
		int affected = 0;
		synchronized (getDBLock()) {
			affected = mSqLiteDatabase
					.delete(table,
							(where != null && where.first != null ? where.first
									: null),
							(where != null && where.first != null
									&& where.second != null ? where.second
	 ...
	 * @return
	 */
	public long insert(String table_name, HashMap<String, String> row,
			boolean replace) {
		long ret = 0;
		...
		synchronized (getDBLock()) {
			try {
				if (replace) {
					ret = mSqLiteDatabase.replace(table_name, null,
							parseRowdata(row));
				} else {
					ret = mSqLiteDatabase.insert(table_name, null,
							parseRowdata(row));
				}
			} catch (Exception e) {
				e.printStackTrace();
	 ...
	 * @return
	 */
	public int update(HashMap<String, String> row,
			Pair<String, ArrayList<String>> where) {
		int ret = 0;
		synchronized (getDBLock()) {
			try {
				ret = mSqLiteDatabase.update(
						table,
						parseRowdata(row),
						where != null ? where.first : null,
						where != null && where.second != null ? where.second
	 ...
	 * @return 找不到数据时返回null
	 */
	public HashMap<String, String> findOne(
			Pair<String, ArrayList<String>> where, String order) {
	 ...
	 */
	public ArrayList<HashMap<String, String>> findAll(
			Pair<String, ArrayList<String>> where, int limit) {
	 ...
	 */
	public ArrayList<HashMap<String, String>> findAll(
			Pair<String, ArrayList<String>> where, String order, int limit) {
		return findAll(where, order, 0, limit);
	}

	public ArrayList<HashMap<String, String>> findAll(
			Pair<String, ArrayList<String>> where, String order, int start,
			int offset) {
	 ...
	 * @return 无符合记录时返回null
	 */
	public ArrayList<HashMap<String, String>> findAll(String table,
			Pair<String, ArrayList<String>> where, String order, int start,
			int offset) {
		...
		synchronized (getDBLock()) {
			...
			try {
				String sql = "select * from "
						+ table
						+ " "
						+ (where != null ? "where " + where.first : "")
						+ " "
						+ (order != null && order.length() > 0 ? "order by "
								+ order : "") + " limit " + start + ","
						+ offset;
				cur = mSqLiteDatabase.rawQuery(
						sql,
						where != null && where.second != null ? where.second
								.toArray(new String[] {}) : null);
				if (cur.getCount() > 0) {
	 ...
	 * @return 无符合记录时返回null
	 */
	public ArrayList<HashMap<String, String>> findAll2(String table,
			Pair<String, ArrayList<String>> where, String order, int limit,
			int offset) {
		ArrayList<HashMap<String, String>> rows = new ArrayList<HashMap<String, String>>();
		synchronized (getDBLock()) {
	...
	private class DatabaseHelper extends SQLiteOpenHelper {
			...
			synchronized (getDBLock()) {
				...
				do {
					...
					try {
						...
						&& System.currentTimeMillis() - time < TIMEOUT * 1000l);
		...
		ArrayList<String> getTables(SQLiteDatabase db) {
			Cursor cur = db
					.rawQuery(
							" select * from sqlite_master where type = 'table' limit 1000",
							null);
			...
			if (cur.getCount() > 0) {
				...
				do {
					...
					if (name.length() < 7
							|| (!name.substring(0, 7).equals("sqlite_"))) {
						arr.add(name);
					}
				} while (cur.moveToNext());
			}
		...
		void clearOldTables(SQLiteDatabase sqlite) {
			...
			if (tables != null) {
				...
				for (int i = 0; i < size; i++) {
					...
					if (!name.substring(name.length() - suffix.length())
							.equals(suffix)) {
		...
		void makeSureNewTables(SQLiteDatabase sqlite, int ver) {
			...
			for (int i = 0; i < size; i++) {
				String name = tables.get(i);
				if (name.substring(name.length() - suffix.length()).equals(
						suffix)) {
		...
		void update2NewTables(SQLiteDatabase sqlite, int oldVersion,
				int newVersion) {
			ArrayList<String> tables = getTables(sqlite);
			...
			for (int i = 0; i < size; i++) {
				...
				if (name.substring(name.length() - suffix.length()).equals(
						suffix)) {
					sqlite.execSQL("alter table " + tables.get(i)
							+ " rename to " + name.replace(suffix, suffix1));
				}
			}
		}
